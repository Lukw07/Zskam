...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        ...
        if (is_admin() || $reservation['user_id'] == $_SESSION['user_id']) {
            ...
            $stmt->bind_param("i", $reservation_id);
            if ($stmt->execute()) {
                $success = "Rezervace byla úspěšně smazána";
            } else {
                $error = "Chyba při mazání rezervace";
            }
        } else {
            $error = "Nemáte oprávnění smazat tuto rezervaci";
        }
    } else {
        ...
        if ($date_obj === false) {
            ...
            // Kontrola dostupnosti (při úpravě nezapočítáváme aktuální rezervaci)
            $stmt = $conn->prepare("SELECT 
                (SELECT total_quantity FROM devices WHERE id = ?) - 
                COALESCE(SUM(CASE WHEN id != ? THEN quantity ELSE 0 END), 0) AS available
                FROM reservations 
                WHERE device_id = ? AND date = ? AND hour = ?");
            $stmt->bind_param("iiisi", $device_id, $reservation_id, $device_id, $date, $hour);
            $stmt->execute();
            ...
            if ($day_of_week >= 6) {
                ...
                if ($reservation_id) {
                    ...
                    $stmt = $conn->prepare("UPDATE reservations 
                        SET device_id = ?, date = ?, hour = ?, quantity = ?
                        WHERE id = ? AND (user_id = ? OR ? = 'admin')");
                    $is_admin = is_admin() ? 'admin' : 'user';
                    $stmt->bind_param("isiiisi", $device_id, $date, $hour, $quantity, $reservation_id, $_SESSION['user_id'], $is_admin);
                    if ($stmt->execute()) {
                        $success = "Rezervace byla úspěšně upravena";
                    } else {
                        $error = "Chyba při úpravě rezervace";
                    }
                } else {
            ...
            <div class="card-body">
                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        <tbody>
                            <?php while ($hour = $hours->fetch_assoc()): 
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <?php
                                    // Kontrola existujících rezervací pro tuto hodinu
                                    $stmt = $conn->prepare("SELECT r.*, u.name as user_name 
                                                          FROM reservations r 
                                                          JOIN users u ON r.user_id = u.id 
                                                          WHERE r.device_id = ? AND r.date = ? AND r.hour = ?");
                                    $stmt->bind_param("isi", $selected_device, $selected_date, $hour['hour_number']);
                                    $stmt->execute();
                                    $existing_reservations = $stmt->get_result();
                                    
                                    $button_shown = false;
                                    while ($reservation = $existing_reservations->fetch_assoc()):
                                        // Kontrola, zda je rezervace v budoucnosti
                                        $reservation_date = new DateTime($reservation['date']);
                                        $current_date = new DateTime();
                                        $is_future = $reservation_date > $current_date;
                                        
                                        if (!$button_shown && (is_admin() || $reservation['user_id'] == $_SESSION['user_id']) && $is_future):
                                    ?>
